...
// POST /api/quotes/[id]/document — joindre après coup le PDF d'un devis créé
// à la main. Multipart : champ « file ». Le PDF est archivé au même endroit
// que ceux des imports ; aucune lecture IA, on ne touche pas aux champs saisis.
export async function POST(
  ...
  try {
    ...
    if (user.role === "READER") {
      return NextResponse.json({ error: "Vous n'avez pas les droits pour modifier un devis" }, { status: 403 });
    }
    const effectiveOrgId = await getEffectiveOrganizationId(user.id, user.organizationId);
    const { id } = await params;

    const limit = await rateLimit(`quotes-document:${user.id}`, "import");
    ...
    if (!quote.contractId) {
      return NextResponse.json({ error: "Ce devis n'est rattaché à aucun contrat" }, { status: 400 });
    }
    ...
    const documentUrl = await archiveQuotePdfToR2(buffer, file.name, quote.contractId, effectiveOrgId);
    if (!documentUrl) {
      return NextResponse.json({ error: "Archivage du PDF impossible, réessayez" }, { status: 502 });
    }

    await prisma.quote.update({ where: { id: quote.id }, data: { documentUrl } });
    return NextResponse.json({ documentUrl });
  } catch (error) {
    console.error("Error attaching quote document:", error);
