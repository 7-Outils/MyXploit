...
export async function GET(request: NextRequest) {
  try {
    ...
    const equipments = await prisma.equipment.findMany({
      where,
      // `select` explicite plutôt qu'`include` : cette réponse alimente tout
      // l'onglet Équipements (liste, cartes, matrice, export PDF, fiche
      // d'édition). Les colonnes que personne ne lit — installDate, quantity,
      // theoreticalLifespan, serviceArea, createdAt/updatedAt, organizationId —
      // pesaient sur chaque ligne d'un parc de plusieurs milliers d'organes.
      select: {
        id: true,
        name: true,
        domain: true,
